...
func (s *CpuSubSystem) Set(cgroupPath string, res *ResourceConfig) error {
	...
	if res.CpuShare != "" {
		if err = os.WriteFile(path.Join(subsysCgroupPath, "cpu.shares"), []byte(res.CpuShare), Perm0644); err != nil {
	...
	if res.CpuCfsQuota != 0 {
		...
		if err = os.WriteFile(path.Join(subsysCgroupPath, "cpu.cfs_period_us"), []byte("100000"), Perm0644); err != nil {
			return fmt.Errorf("set cgroup cpu share fail %v", err)
		}
		...
		if err = os.WriteFile(path.Join(subsysCgroupPath, "cpu.cfs_quota_us"), []byte(strconv.Itoa(PeriodDefault/Percent*res.CpuCfsQuota)), Perm0644); err != nil {
			return fmt.Errorf("set cgroup cpu share fail %v", err)
		}
	}
	return nil
...
func (s *CpuSubSystem) Apply(cgroupPath string, pid int, res *ResourceConfig) error {
	...
	if err = os.WriteFile(path.Join(subsysCgroupPath, "tasks"), []byte(strconv.Itoa(pid)), Perm0644); err != nil {
